...
function Luna:CreateWindow(WindowSettings)
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateInput(InputSettings, Flag)
			...
			-- Input fix: make the TextBox explicitly focusable/editable.
			-- Some executor-loaded Luna UI templates have these properties
			-- disabled or inherited incorrectly from the packaged UI asset.
			...
			-- Make the entire input area clickable, not just the TextBox.
			local function FocusInput()
				if InputBox and InputBox:IsDescendantOf(game) then
			...
			Input.InputFrame.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1
					or input.UserInputType == Enum.UserInputType.Touch then
					FocusInput()
				end
			end)
			InputBox.ZIndex = math.max(InputBox.ZIndex, Input.InputFrame.ZIndex + 1)
